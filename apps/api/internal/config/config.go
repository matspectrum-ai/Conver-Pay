...
type Config struct {
	...
	ProviderCredentialsMasterKey string
...
func Load() (Config, error) {
	cfg := Config{
		...
		ProviderCredentialsMasterKey: strings.TrimSpace(os.Getenv("PROVIDER_CREDENTIALS_MASTER_KEY")),
		ProviderHTTPTimeout:          durationOrDefault("PROVIDER_HTTP_TIMEOUT", 10*time.Second),
	}
	...
	if cfg.AppEnv == "production" && cfg.ProviderCredentialsMasterKey == "" {
		return Config{}, fmt.Errorf("PROVIDER_CREDENTIALS_MASTER_KEY is required when APP_ENV=production")
	}

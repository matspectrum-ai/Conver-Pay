...
export function DashboardShell() {
  ...
  const navigatePayment = useCallback((direction: -1 | 1) => {
    setSelectedPayment((current) => {
      if (!current) return current;
      const index = payments.findIndex((item) => item.id === current.id);
      const nextIndex = (index + direction + payments.length) % payments.length;
      return payments[nextIndex];
    });
  }, []);
      ...
      <main className="content-shell">
        ...
        <div className="page-content">
          ...
          <section className="hero-row">
            <div><p className="eyebrow">Sunday, September 7</p><h1>Payment network is healthy.</h1><p className="hero-copy">Conver is automatically routing Pix traffic across 3 connected providers.</p></div>
          ...
          <section className="operations-grid">
            <div className="panel routing-panel"><div className="panel-header"><div><span className="section-kicker">AUTOMATIC ROUTING</span><h2>Live payment flow</h2></div><button className="ghost-button">Routing details <Icons.arrow className="icon" /></button></div><RoutingMap /></div>
            <div className="panel provider-panel">
              ...
              <div className="provider-list">{providers.map((provider) => <div className="provider-row" key={provider.id}><div className="provider-identity"><span className={`provider-logo provider-${provider.id}`}>{provider.name.slice(0,1)}</span><div><strong>{provider.name}</strong><span>{provider.share}% traffic</span></div></div><div className="provider-score"><strong>{Math.round(provider.score * 100)}</strong><span>score</span></div><div className="provider-meta"><span>{provider.latency} ms</span><span>{provider.success}% QR</span></div><span className={`health-dot ${provider.state}`} /></div>)}</div>
              <button className="panel-footer-link">View provider observability <Icons.arrow className="icon" /></button>
            </div>
          </section>

          <section className="secondary-grid">
            <div className="panel recovery-panel"><div className="panel-header compact"><div><span className="section-kicker">RECOVERIES</span><h2>Revenue protected</h2></div><span className="live-label"><span className="live-dot" /> live</span></div><div className="recovery-list">{recoveries.map((recovery) => <div className="recovery-row" key={recovery.id}><div className="recovery-icon">↗</div><div className="recovery-main"><strong>{recovery.amount}</strong><span>{recovery.from} <b>→</b> {recovery.to}</span></div><span className="recovery-reason">{recovery.reason}</span><time>{recovery.age}</time></div>)}</div></div>
            <div className="panel insight-panel"><div className="insight-orb"><span /><span /><span /></div><span className="section-kicker">ROUTING INSIGHT</span><h2>BlackCat is degrading</h2><p>p95 latency rose 63% in the last 15 minutes. Traffic was reduced from 31% to 18% automatically.</p><button className="ghost-button">Inspect decision <Icons.arrow className="icon" /></button></div>
          </section>
          ...
          <section className="payments-section">
            ...
            <div className="table-wrap"><table><thead><tr><th>Payment</th><th>Order</th><th>Amount</th><th>Provider</th><th>Status</th><th>QR latency</th><th></th></tr></thead><tbody>{payments.map((payment) => <tr key={payment.id}><td><button className="mono-link" onClick={() => setSelectedPayment(payment)}>{payment.id}</button><span className="row-age">{payment.age}</span></td><td className="muted-cell">{payment.order}</td><td className="amount-cell">{payment.amount}</td><td><span className="provider-inline"><i />{payment.provider}</span></td><td><Status status={payment.status} /></td><td className="muted-cell">{payment.latency}</td><td><button className="row-action" aria-label={`Inspect ${payment.id}`} onClick={() => setSelectedPayment(payment)}>›</button></td></tr>)}</tbody></table></div>
          </section>
          </> : <SectionPreview section={active} onOpenPayment={setSelectedPayment} />}
        </div>
      </main>
